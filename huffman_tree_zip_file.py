...
# copyright@zhanggugu
import six
#第三方库，用在 byte 和 int 类型之间的转换，在压缩和解压缩文件当中会用到
...
class HuffTree(object):
    ...
    def traverse_huffman_tree(self, root, code, char_freq):
        ...
        if root.isleaf():
            ...
            print(("it = %d/%c and  freq = %d  code = %s") % (
            root.get_value(), chr(root.get_value()), root.get_wieght(), code))
            return None
        else:
...
if __name__ == '__main__':
    ...
    # INPUTFILE： 输入的文件名
    # OUTPUTFILE：输出的文件名
    FLAG = input('请选择压缩或解压（0代表压缩，1代表解压）：')
    INPUTFILE = input('请输入压缩文件名：')
    OUTPUTFILE = input('请输入解压后的文件名：')
